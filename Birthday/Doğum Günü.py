...
def dogumgGunu(günler):
    if günler > 0:
        print(f"Dğum gününüz {günler} gün önce idi😊.")
    elif günler < 0:
        print(f"Dğum gününüz {günler} sonra 😊.")
    else:
        print(f"Dğum günün kutlu olsun 😊.")
...
def main():
    ...
    while durum:
        try:
            ...
            dogumgGunu(ogün)
            print(f"{toplamGün}, gündür hayattasınız tebrikler...😊 ")
        except Exception as ex:
            print(f"Lütfen geçerli değerler girin. Hata kodu {ex}")
        else:
            durum = False
